...
to setup
  ...
  set explosion-power explosion-power
  setup-city
  setup-people
...
to go
  if explosion-power <=  0 [stop] ;; Stop als de radiatie groot genoeg is
  expand-explosion
  check-damage
...
to expand-explosion
  set explosion-radius explosion-radius + explosion-power
  set radiation-radius radiation-radius + (1.5 * explosion-power) ;; Radiatie breidt sneller uit
  ask patches [
    ...
    if pcolor = white [
      set explosion-power explosion-power - 0.01
    ]
    if pcolor = gray[
      set explosion-power explosion-power - 0.05
    ]
